import { getUserByEmail } from "@/lib/services/users";

import { CustomResponse } from "@/lib/utils/CustomResponse";
import AppError from "@/lib/exception/AppError";

import HttpStatusCode from "@/lib/constants/HttpStatusCode";
import Messages from "@/lib/constants/Messages";

import type { NextRequest, NextResponse } from "next/server";
import { loginFormSchema } from "@/lib/constants/FormSchema/loginFormSchema";
import { LoginBodyType, UserType } from "@/types";
import { validateData } from "@/lib/utils/ValidateData";

export async function POST(req: NextRequest): Promise<NextResponse> {
  try {
    const body: LoginBodyType = await req.json();

    validateData<LoginBodyType>(loginFormSchema, body);
    const user = await getUserByEmail(body.email);

    if (user.password !== body.password) {
      throw new AppError(
        Messages.LOGIN_DATA_INVALID,
        HttpStatusCode.UNAUTHORIZED
      );
    }

    return CustomResponse<Pick<UserType, "id">>({
      body: { message: Messages.LOGIN_SUCCESS, data: { id: user.id } },
    });
  } catch (error) {
    if (error instanceof AppError) {
      if (error.isOperational) {
        return CustomResponse({
          statusCode: error.statusCode,
          body: { message: error.message },
        });
      }
    }

    return CustomResponse({
      statusCode: HttpStatusCode.INTERNAL_SERVER_ERROR,
      body: { message: Messages.INTERNAL_SERVER },
    });
  }
}

...
import type { RegisterBodyType, UserType } from "@/types";
...
export const getUserById = async (id: string): Promise<UserType> => {
  const { data, error } = await supabase
    ...
    .maybeSingle();
  if (!data || error)
    throw new AppError(Messages.USER_NOT_FOUND, HttpStatusCode.NOT_FOUND);

  return data;
};

export const addUser = async (
  data: Omit<RegisterBodyType, "confirm_password">
): Promise<boolean> => {
  const isUserExist = await checkUserExistByEmail(data.email);
  if (isUserExist)
    throw new AppError(Messages.REGISTERED_EMAIL, HttpStatusCode.CONFLICT);
